...
- Runs the same K sweep used for the QNN Shapley reproduction:
    K in {1, 8, 16, 32}, alpha=1.0, 5 independent runs.
...
def load_or_create_dataset(data_path: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if not data_path.exists():
        raise FileNotFoundError(
            ...
            "Pass --data-path data/qnn-data.csv"
        )
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=Path, default=repo_root / "results" / "qnn_owen_emx_main")
