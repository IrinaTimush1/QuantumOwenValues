...
def plot_combined_gate_values(output_dir: Path) -> Tuple[Path, Path]:
    """Create the paper Figure-6-style combined QSVM Owen plot."""
    rows: List[pd.DataFrame] = []
    for r in [1, 2, 3]:
        path = output_dir / f"r{r}" / "gate_owen_values.csv"
        if path.exists():
            rows.append(pd.read_csv(path))
    if not rows:
        raise FileNotFoundError(f"No gate_owen_values.csv files found under {output_dir}")

    with plt.rc_context(PAPER_STYLE):
        fig, ax = plt.subplots(figsize=(11.8, 4.0))
        for df in rows:
            r = int(df["r"].iloc[0])
            style = PAPER_SERIES_STYLE.get(r, PAPER_SERIES_STYLE[1])
            active_df = df[df["group"] != "passive"].copy()
            ax.scatter(
                active_df["gate_index_1based"].to_numpy(dtype=int),
                active_df["owen_value"].to_numpy(dtype=float),
                marker=style["marker"],
                s=35,
                color=style["color"],
                label=style["label"],
        ...
        # H gates are locked/passive in the Owen experiment. Show them at zero
        # once, rather than assigning them a player value.
        passive_h = [g for g in range(1, 22) if qsvm_gate_name(g) == "H"]
        ax.scatter(
            passive_h,
            np.zeros(len(passive_h)),
            marker="x",
            s=28,
            color="0.45",
            linewidths=0.9,
        ...
        png = plots_dir / "fig_qsvm_owen_gate_values_combined.png"
        pdf = plots_dir / "fig_qsvm_owen_gate_values_combined.pdf"
        save_figure(fig, png, pdf)
        return png, pdf


def plot_combined_value_distributions(output_dir: Path) -> Tuple[Path, Path]:
    """Create the paper Figure-7-style multi-panel value-distribution plot."""
    dist: Dict[int, pd.DataFrame] = {}
    for r in [1, 2, 3]:
        path = output_dir / f"r{r}" / "all_subset_value_distribution.csv"
        if path.exists():
            dist[r] = pd.read_csv(path)
    missing = [r for r in [1, 2, 3] if r not in dist]
    if missing:
        raise FileNotFoundError(
            f"Missing all_subset_value_distribution.csv for r={missing} under {output_dir}"
        )
    ...
    with plt.rc_context(PAPER_STYLE):
        ...
        _plot_value_distribution_axis(
            ...
            dist_df=dist[1],
            r=1,
            panel_label=r"(a) $r=1$, exact Owen",
            show_ylabel=True,
            show_passive_note=False,
        )
        _plot_value_distribution_axis(
            ax=ax2,
            dist_df=dist[2],
            r=2,
            panel_label=r"(b) $r=2$, exact Owen",
            show_ylabel=True,
            show_passive_note=False,
        )
        _plot_value_distribution_axis(
            ...
            dist_df=dist[3],
            r=3,
            panel_label=r"(c) $r=3$, exact Owen",
            show_ylabel=True,
            show_passive_note=False,
        )
        fig.text(
            0.5,
            0.02,
            "H gates are passive and always included; k is total gates, so bins below the passive-H count are empty.",
            ha="center",
            fontsize=10,
        )
        plots_dir = ensure_dir(output_dir / "plots")
        png = plots_dir / "fig_qsvm_value_distribution_combined.png"
        pdf = plots_dir / "fig_qsvm_value_distribution_combined.pdf"
...
def add_owen_args(parser: argparse.ArgumentParser) -> None:
